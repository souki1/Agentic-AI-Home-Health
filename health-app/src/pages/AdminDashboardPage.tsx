...
import { KpiCard, LineChartCard, DataTable, StatusBadge } from '../components';
...
export function AdminDashboardPage() {
  ...
  const getPatientCondition = (id: string) => patients.find((p) => p.id === id)?.condition ?? '—';
    ...
    <AppLayout role="admin" email={user?.email ?? ''} pageTitle="Dashboard">
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard title="Missing Check-ins Rate (7d)" value={missingRate7d} subtitle="Last 7 days" />
          <KpiCard title="Flagged Check-ins (7d)" value={flagged7d} subtitle="Needs follow-up or escalated" />
          <KpiCard title="Avg Symptom Score (7d)" value={avgSymptom7d} />
        </div>
        <div className="grid gap-6 lg:grid-cols-2">
          <LineChartCard title="Check-ins per day (30 days)" data={byDay} valueLabel="Check-ins" />
          <LineChartCard title="Avg risk score trend (30 days)" data={riskByDay} valueLabel="Risk score" color="#dc2626" />
        </div>
        <div>
          <h2 className="mb-3 text-lg font-semibold text-slate-800">Needs Follow-up</h2>
          <DataTable
            keyField="id"
            data={needsFollowUp}
            columns={[
              { key: 'patient', header: 'Patient', render: (r) => getPatientName(r.patient_id) },
              { key: 'condition', header: 'Condition', render: (r) => getPatientCondition(r.patient_id) },
